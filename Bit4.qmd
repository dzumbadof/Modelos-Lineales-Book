...
En la @fig-serieDefunciones se observa que la serie de defunciones totales no es estacionaria, pues tiene una tendencia creciente muy clara. Esto se confirma con el gráfico en la @fig-ACFDefunciones que muestra que la decadencia de las correlaciones no es suficientemente rápida (lineal). Se lleva a cabo un diferencia para tratar de eliminar la tendencia.
...
#| fig-cap-location: top
#| echo: false
plot(diff(serie), main="", xlab="Tiempo")
```
En la @fig-serieDefuncionesDif no queda claro que la tendencia ha sido eliminado, de hecho parece ser creciente. Más aún, la varianza no aparenta estable, pues se nota más varianza a partir de 1998. 

```{r}
#| fig-cap: "Serie de tiempo de la tasa de crecimiento de defunciones"
#| label: fig-serietasaDefunciones
#| fig-cap-location: top
#| echo: false
plot(diff(log(serie)), main="", xlab="Tiempo")
```
La serie resultante mostrada en la @fig-serietasaDefunciones parece ser estable, estacionaria e interpretable como la tasa de crecimiento porcentual de las defunciones. Esto se confirma al observa el ACF y PACF en la @fig-ACFtasaDefunciones

```{r}
#| fig-cap: "ACF y PACF de la serie de tasa de crecimiento de defunciones"
#| label: fig-ACFtasaDefunciones
#| fig-cap-location: top
#| echo: false
t<-acf2(diff(log(serie)), main="", max.lag = 15)
```
...
```{r}
#| echo: false
#| fig-cap: 'Diagnósticos del modelo ARIMA(0,1,1)'
#| label: fig-diagMa1
#| message: false
modelo1 <- sarima(log(serie), 0,1,1)
```

```{r}
#| echo: false
#| fig-cap: 'Diagnósticos del modelo ARIMA(1,1,0)'
#| label: fig-diagAR1
#| message: false
modelo2 <- sarima(log(serie), 1,1,0)
```

```{r}
#| echo: false
#| fig-cap: 'Diagnósticos del modelo ARIMA(1,1,1)'
#| label: fig-diagARIMA111
#| message: false
modelo3 <- sarima(serie, 1,1,1)
```
Se observa en los diagnósticos que los tres modelos propuestos logran aprobar contundentemente el supuesto de independencia de los residuos mostrado en los gráficos de ACF que ningún rezago tiene significancia. Aún más evidencia para este supuesto es proporcionado por el valor p de la pruebga de Ljung-Box donde se observa un no rechazo de la independencia en los tres modelos. Al observa el qqplot de los residuos en los modelos MA Y AR se observa que la cola izquierda podría ser un problema para la hipótesis de normal, pues estos están al borde de desviarse significativamente de los cuantiles teóricos de la normal. 
...
```{r}
#| fig-cap: 'Pronóstico del modelo ARIMA(1,1,0) de la tasa de defunciones '
#| label: fig-pronosticoARIMA
#| echo: false
def.for <- sarima.for(log(serie), n.ahead = 2, 1,1,0)
```

```{r}
#| echo: false
# Intervalos de confianza
U95 <- exp(def.for$pred + def.for$se*qnorm(1-0.05/2))
L95 <- exp(def.for$pred - def.for$se*qnorm(1-0.05/2))

U80 <- exp(def.for$pred + def.for$se*qnorm(1-0.2/2))
L80 <- exp(def.for$pred - def.for$se*qnorm(1-0.2/2))


for.int <- data.frame(c(2019, 2020), exp(def.for$pred), L80, U80, L95, U95)
```
...
```{r}
#| echo: false
abs( as.numeric(exp(def.for$pred)) - base$defunciones[70:71])
```
