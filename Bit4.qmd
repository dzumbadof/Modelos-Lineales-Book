...
En la bitácora anterior se hizo un ajuste de un modelo ARIMA a la serie de defunciones totales. En esta bitácora se hará nuevamente, pero esta vez de manera más descriptivo y completa. Además que se añade un componente de comparación en modelos, el cual hizo falta en la bitácora pasada. Primer, la @fig-serieDefunciones muestra la serie de tiempo de defunciones totales. Se muestra un comportamiento creciente. El objetivo es ajustar un modelo de la forma 
...
Se observa de la @fig-diagAR1 y la @fig-diagARMA02 que los modelos con componente AR(1) y MA(2) obtienen residuos que parecen ser independientes según el ACF. También, salvo un outlier en el primer modelo, también se puede decir que los residuos aparentan ser normales según el qqplot. El modelo con componente MA(2) muestra evidencia de dependencia en los primeros dos rezagos y cola derecha de los residuos que parece no ser normal. Más aún, al observar el valor p de la pruba de Ljung-Box se nota que este modelo presenta alta evidencia de que hay dependencia residual. El modelo MA(2), salvo los primeros dos rezagos parece cumplir con la hipótesis nula de independencia de la prueba de Ljung-Box, mientras que el modelo AR(1) tiene un no rechazo contundente bajo esta prueba. 
